...
export const Music = () => {
  ...
  return (
    <>
      {datas && datas.map((data: any, _index: any) => (
        <div className=' p-1 h-24 w-64  border rounded-xl flex  gap-x-3'>
          <img src={data?.image} className=' w-2/5 h-full border rounded-xl' />
          <div className=' flex flex-col gap-y-3 w-full flex-1 '>
            <p className=' text-[15px] font-bold'>{data?.song || 'yayuya'}</p>
            <div className=' flex flex-col gap-y-3'>
              <p className=' text-[12px] font-medium'>{data?.singer || 'jytjkg'}</p>
              <p className=' text-[12px]'>{data.popularity || 'aidyiky'}</p>
            </div>
